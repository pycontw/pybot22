...
sudo docker run --network my-network -v "$PWD":/pybot22 --env-file=./env.var -d --name pycat-bot pycat
...
echo "  $ sudo docker run -it --network my-network -v "$PWD":/pybot22 --env-file=./env.var pycat /bin/bash"
